...
Int lsum(int l) { return Int(3).pow(l); }

Int rec(const string &s, int head, int length) {
  if (length == 0) return 1;
  if (s[head] == '0') return rec(s, head + 1, length - 1);
  Int result = rec(s, head + 1, length - 1) * Int(2);
  result += lsum(length - 1);
  return result;
}
