...
int main() {
  ...
  while (cin >> n >> k) {
    ...
    for (auto &e: m) v.push_back(make_pair(e.first, e.second));
    
    static Int dp[N][N];
    REP(i,N) REP(j,N) dp[i][j] = 0;
    dp[0][0] = Int(1);
    FOR(i,1,v.size()+1) REP(j,k+1) {
      dp[i][j] += dp[i-1][j];
      if (j) dp[i][j] += dp[i-1][j-1] * Int(v[i-1].second);
    }

    // REP(i,n+1) pp(dp[i],k+1);
